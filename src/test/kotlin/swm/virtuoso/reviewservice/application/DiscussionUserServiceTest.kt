...
class DiscussionUserServiceTest {
    ...
    fun `markDiscussionAsMention should mark user as mentioned`() {
        ...
        doReturn(mock(Discussion::class.java)).`when`(discussionPort).findDiscussion(discussionId)
        doReturn(mock(UserEntity::class.java)).`when`(giteaPort).findUserById(userId)
        doReturn(findDiscussionUser).`when`(discussionUserPort).findDiscussionUser(userId)
        ...
        // then
        verify(discussionPort).findDiscussion(discussionId)
        verify(giteaPort).findUserById(userId)
        verify(discussionUserPort).findDiscussionUser(userId)
